...
def plot_stats():
    """Generate and display statistics plots."""
    print("Generating Growth Mindset Statistics...")
    ...
    try:
        with open(os.path.join(data_dir, 'quotes.json'), 'r') as f:
            quotes_data = json.load(f)
        with open(os.path.join(data_dir, 'user_progress.json'), 'r') as f:
            progress_data = json.load(f)
    except FileNotFoundError:
        print("No data files found. Please run the application first to generate data.")
        return
    ...
    # Plot 1: Quote Categories Distribution
    plt.figure(figsize=(12, 6))
    if quotes_data:
        categories = [tag for quote in quotes_data.values() for tag in quote.get('tags', [])]
        category_counts = Counter(categories)
        
        plt.subplot(1, 2, 1)
        sns.barplot(x=list(category_counts.values()), y=list(category_counts.keys()))
        plt.title('Quote Categories Distribution')
        plt.xlabel('Count')
        
    # Plot 2: User Progress Over Time
    if progress_data:
        dates = list(progress_data.keys())
        scores = list(progress_data.values())
        
        plt.subplot(1, 2, 2)
        sns.lineplot(x=range(len(dates)), y=scores)
    ...
    # Save the plots
    plt.tight_layout()
    plot_path = os.path.join(plots_dir, 'mindset_stats.png')
    plt.savefig(plot_path)
    plt.close()
    
    print(f"Statistics plots have been saved to: {plot_path}")
    print("Plots include:")
